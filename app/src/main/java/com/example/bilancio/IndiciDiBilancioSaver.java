...
public class IndiciDiBilancioSaver extends AsyncTask<Workbook, Void, Boolean> {

    private Context context;

    public IndiciDiBilancioSaver(Context context) {
    ...
    protected Boolean doInBackground(Workbook... workbooks) {
        try {
            ...
            File file = new File(folder, "indicidibilancio.xlsx");
            int count = 1;
            while (file.exists()) {
                String filename = "indicidibilancio(" + count + ").xlsx";
    ...
    protected void onPostExecute(Boolean result) {
        if (result) {
            Toast.makeText(context, "INDICI DI BILANCIO SALVATI CORRETTAMENTE", Toast.LENGTH_SHORT).show();
        } else {
